            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body animatable table-responsive">
                    <table id="datatable" class="table table-hover pr-0 pl-0 "
                        ...
                        <tbody>
                        <x-blank-table-indicator columns="5" :data="$genres"/>
                        @foreach($genres as $genre)
                            <tr id="genre_row_{{$genre->getKey()}}">
                                <td>{{$loop->index+1}}</td>
                                <td>
                                    @if($genre->poster!=null)
                                        <img src="{{$genre->poster}}" style="max-height: 100px" alt="{{$genre->name}}"
                                             class="img-thumbnail" @include('admin.extras.tooltip.right', ['title' => $genre->name])/>
                                    @else
                                    ...
                                    <div class="btn-group btn-group-toggle shadow-sm" data-toggle="buttons">
                                        @if($genre->active==true)
                                            <label class="btn btn-outline-danger active" @include('admin.extras.tooltip.left', ['title' => 'Set genre active'])>
                                                <input type="radio" name="options" id="optionOn_{{$genre->getKey()}}"
                                                       onchange="_status('{{$genre->getKey()}}',1);"/> On
                                            </label>
                                            <label class="btn btn-outline-primary" @include('admin.extras.tooltip.right', ['title' => 'Set genre inactive'])>
                                                <input type="radio" name="options" id="optionOff_{{$genre->getKey()}}"
                                                       onchange="_status('{{$genre->getKey()}}',0);"/> Off
                                            </label>
                                        @else
                                    ...
                                    <div class="btn-toolbar" role="toolbar">
                                        <div class="btn-group mx-auto" role="group">
                                            <a class="btn btn-outline-danger"
                                               href="{{route('admin.genres.edit',$genre->getKey())}}" @include('admin.extras.tooltip.bottom', ['title' => 'Edit'])><i
                                                        class="mdi mdi-pencil"></i></a>
                                            <a class="btn btn-outline-primary"
                                               href="javascript:_delete('{{$genre->getKey()}}');" @include('admin.extras.tooltip.bottom', ['title' => 'Delete'])><i
