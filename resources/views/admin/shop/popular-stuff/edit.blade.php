		...
		<div class="col-12">
			<div class="card shadow-sm">
				<div class="card-header py-0">
					@include('admin.extras.header', ['title'=>'Modify Popular Stuff Section'])
				</div>
				<form action="{{route('admin.shop.popular-category.update')}}" method="post">
					@csrf
							...
							<div class="col-8 mx-auto">
								<div class="card shadow-sm border animated slideInLeft">
									<div class="card-header">
										<div class="row">
											<div class="col-8 my-auto">Choose upto 5 categories</div>
											...
											@foreach($categories as $topLevel)
												<li>
													<div class="custom-control custom-checkbox mr-sm-2" data-name="{{$topLevel['name']}}">
														<input type="checkbox" name="choice[]" class="custom-control-input" id="check_{{$topLevel['id']}}" onchange="handleStateChanged();" @if($topLevel['popularCategory']) checked @endif value="{{$topLevel['id']}}">
															...
															@foreach($topLevel['inner'] as $inner)
																<li>
																	<div class="custom-control custom-checkbox mr-sm-2" data-name="{{$inner['name']}}">
																		<input type="checkbox" name="choice[]" class="custom-control-input" id="check_{{$inner['id']}}" onchange="handleStateChanged();" @if($inner['popularCategory']) checked @endif value="{{$inner['id']}}">
																			...
																			@foreach($inner['inner'] as $innerNext)
																				<li>
																					<div class="custom-control custom-checkbox mr-sm-2" data-name="{{$innerNext['name']}}">
																						<input type="checkbox" name="choice[]" class="custom-control-input" id="check_{{$innerNext['id']}}" onchange="handleStateChanged();" @if($innerNext['popularCategory']) checked @endif value="{{$innerNext['id']}}">
...
@section('javascript')
	<script type="application/javascript">
		...
		handleStateChanged = () => {
			...
			if (checked) {
				if (count >= 5) {
					alertify.log('Only 5 categories are allowed.');
