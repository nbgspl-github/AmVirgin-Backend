(function (window) {
	...
	c3_chart_internal_fn.redrawSubchart = function (withSubchart, transitions, duration, durationForExit, areaIndices, barIndices, lineIndices) {
		...
		if (config.subchart_show) {
			...
			// update subchart partials if needed
			if (withSubchart) {
	...
	c3_chart_internal_fn.generateFlow = function (args) {
		...
		return function () {
			...
			// update x domain to generate axis partials for flow
			domain = $$.updateXDomain(targets, true, true);
			// update partials related to x scale
			if ($$.updateXGrid) {
				$$.updateXGrid(true);
			}
			...
			d3.transition().ease('linear').duration(durationForFlow).each(function () {
				...
				.call(wait, function () {
					...
					// draw again for removing flowed partials and reverting attr
					xgrid
						.attr('transform', null)
						.attr($$.xgridAttr);
					xgridLines
	...
	c3_chart_fn.destroy = function () {
		...
		// MEMO: this is needed because the reference of some partials will not be released, then memory leak will happen.
